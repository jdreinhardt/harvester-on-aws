...
RAW_IMAGE_NAME="${PROJECT_PREFIX}-bios-amd64.raw"
ISO_PATH="${ARTIFACTS_DIR}/${PROJECT_PREFIX}-bios-amd64.iso"
...
for f in "$ISO_PATH" "$VMLINUZ_PATH" "$INITRD_PATH"; do
    if [ ! -f "$f" ]; then
...
echo ""
echo "Starting Harvester installation in BIOS mode..."
